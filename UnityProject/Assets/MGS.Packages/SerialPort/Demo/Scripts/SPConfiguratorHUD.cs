...
{
    public class SPConfiguratorHUD : MonoBehaviour
    {
        ...
        private void Start()
        {
            config = SerialPortAPI.Configurator.Config;
        }
        ...
        private void OnGUI()
        {
            ...
            {
                SerialPortAPI.Configurator.WriteCfg();
                Debug.LogFormat("Config is write to the file at path {0}", (SerialPortAPI.Configurator as SPConfigurator).FilePath);
            }
            GUILayout.EndArea();
        }
        #endregion
